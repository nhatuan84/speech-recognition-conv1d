...
class AudioDataset():
    ...
    def __getitem__(self, index):
        ...
        return {"sound": np.expand_dims(wav, axis=1), "label": label}
...
class AudioPredictionDataset():
    ...
    def __getitem__(self, index):
        ...
        wav = librosa.util.normalize(wav)
        wav = librosa.util.pad_center(wav, self.sr)
        return {"sound": np.expand_dims(np.expand_dims(wav, axis=1), axis=0), "filename": filename}
...
def get_sampler(dataset, sample_count=None):
    ...
    # sample_weight = np.array(([float(weights[i]) for i in dataset.labels]))
    # sample_weight /= sample_weight.sum()
    # sample_count = sample_count if sample_count else len(sample_weight)
    # return np.random.multinomial(sample_count, sample_weight, size=1)
